...
textTemplate :: (TemplateQExp Text filling) => Proxy filling QuasiQuoter
textTemplate @filling = Proxy $ QuasiQuoter {
    quoteExp  = textTemplate2QExp @filling . Proxy . DT.pack
   ,quotePat  = undefined
   ,quoteDec  = undefined
   ,quoteType = undefined
}
...
textTemplate2QExp :: (TemplateQExp Text filling)
...
class (ToQExp text,ToQExp filling,HoleFillingExp text filling) => TemplateQExp text filling
...
instance TemplateQExp Text   Int
instance TemplateQExp Text   Double
instance TemplateQExp Text   String
instance TemplateQExp String Text

-- | Convert a `Hole` into a Template Haskell expression.
hole2QExp :: (TemplateQExp text filling) => Proxy text (Hole filling) -> Q Exp
...
iTemplate2QExp :: (TemplateQExp text filling) 
               => ITemplate text 
               -> HoleProps filling 
               -> Q Exp
...
template2QExp :: (TemplateQExp text filling) => Template text filling -> Q Exp
